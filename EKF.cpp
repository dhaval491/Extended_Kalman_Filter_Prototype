...
void Correct(map<std::string,std::vector<long double>> sensor_data, map<int, vector<int>> landmarks)
{
    ...
    x_pos.push_back(mu(0));
    y_pos.push_back(mu(1));
...
vector<double> l_x_pos;
vector<double> l_y_pos;
for(auto it = landmarks.begin();it != landmarks.end(); it++)
{
    l_x_pos.push_back(it->second[0]);
    l_y_pos.push_back(it->second[1]);
}
